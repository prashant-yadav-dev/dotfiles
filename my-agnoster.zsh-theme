...
prompt_end() {
  ...
  echo -n "%{%f%}"
  CURRENT_BG=''
  # Add a newline and the green ❯ symbol
  echo -n $'\n%B%F{green}❯%f%b '
}
...
prompt_context() {
  if [[ "$USERNAME" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    local short_user="@${USERNAME%%.*}"   # take only part before first dot
    prompt_segment "$AGNOSTER_CONTEXT_BG" "$AGNOSTER_CONTEXT_FG" "%(!.%{%F{$AGNOSTER_STATUS_ROOT_FG}%}.)${short_user}"
  fi
}
...
prompt_dir() {
  local dir="%1~"   # show last 2 directories

  # Trim if longer than 30 chars, keep the END (so you see project/folder)
  if [[ ${#dir} -gt 30 ]]; then
    dir="…${dir: -30}"
  fi

  prompt_segment "$AGNOSTER_DIR_BG" "$AGNOSTER_DIR_FG" "$dir"
}
...
prompt_virtualenv() {
  ...
  if [ -n "$CONDA_DEFAULT_ENV" ]; then
    ...
    [[ ${#conda_env} -gt 20 ]] && conda_env="${conda_env:0:20}…"
    prompt_segment magenta $CURRENT_FG "%B🐍 $conda_env%b"
  fi

  # Python virtualenv
  if [[ -n "$VIRTUAL_ENV"  && -d "$VIRTUAL_ENV" ]]; then
    local venv_name="${VIRTUAL_ENV:t:gs/%/%%}"
    local pyver=$(python3 -V 2>&1 | awk '{print $2}' | cut -d. -f1,2)

    # Trim if longer than 20 chars
    [[ ${#venv_name} -gt 20 ]] && venv_name="${venv_name:0:20}…"

    # Red bold venv name + yellow Python version
    prompt_segment default default "%F{red}%B($venv_name%b%f|%F{yellow}$pyver)%f"
  fi
}
